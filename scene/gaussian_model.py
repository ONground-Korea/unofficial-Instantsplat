...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self._world_view_transform = torch.eye(4)
        self._projection_matrix = torch.zeros(4,4)
        self._9D = torch.empty(0)
    ...
    def set_camera(self, camera: Camera):
        ...
        # Optimizer에 텐서를 등록한 후, self._world_view_transform가 optimizer에 등록된 해당 텐서로 대체됨
        optimizable_tensors = self.replace_tensor_to_optimizer_cam(camera.world_view_transform.detach().clone(), "9D")
        # print(optimizable_tensors)
        self._9D = optimizable_tensors["9D"]
        self._world_view_transform = self.r6d2mat(self._9D)
        self._projection_matrix = camera.projection_matrix.detach().clone()
    ...
    def training_setup(self, training_args):
        ...
        l_cam = [
            {'params': [self._9D], 'lr': 0.001, "name": "9D"},
        ]

        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
        self.optimizer_cam = torch.optim.SGD(l_cam, lr=0.0)
    ...
    def load_ply(self, path):
        ...
        self._9D = nn.Parameter(torch.zeros(1,9, device="cuda").requires_grad_(True))
        # self._world_view_transform = nn.Parameter(torch.eye(4, device="cuda").requires_grad_(True))
        # self._projection_matrix = nn.Parameter(torch.zeros(4,4, device="cuda").requires_grad_(True))
    ...
    def replace_tensor_to_optimizer_cam(self, tensor, name):
        optimizable_tensors = {}
        for group in self.optimizer_cam.param_groups:
                if name == "9D":
                    tensor = self.pose_to_d9(tensor.transpose(1,0).unsqueeze(0))
                    group["params"][0] = nn.Parameter(tensor.requires_grad_(True))
                    optimizable_tensors["9D"] = group["params"][0]
