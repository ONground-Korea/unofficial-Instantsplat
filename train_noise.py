...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, output_path, random, num_views, exp_name, project_name, rgb_fix, zero_init, rgb_free, noise, no_densify):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset,output_path, exp_name, project_name)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians,random_gaussian=random,num_views=num_views,rgb_fix=(rgb_fix or rgb_free),zero_init=zero_init, noise=noise)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Densification
            if iteration < opt.densify_until_iter and no_densify==False:
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from,args.output_path,args.random_pointcloud, args.num_views, args.exp_name, args.project_name, args.rgb_fix, args.zero_init, args.rgb_free, args.noise, args.no_densify)
